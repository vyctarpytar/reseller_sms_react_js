...
export default function VerifyNewPhone() {
	...
	const onFinishOtp = async (data) => {
	  ...
	  const resendOTP = async (data) => {
		const res = await dispatch(
		  saveForget({
			url:`api/v1/usr/new-phone-otp/${user?.drftId}?newMobile=${singleloggedInUser?.drftPhoneNo}`,
		  })
		);
		if (res?.payload?.success) {
					...
					<Form
						...
						<div className='grid grid-cols-1'>
							<Form.Item
								...
								label={
									<span>
										Enter OTP sent to +{addSpaces(singleloggedInUser?.drftPhoneNo)}
									</span>
								}
							...
							{counterOtp.minutes === 0 && counterOtp.seconds === 0 ? (
								<button
									...
									Didn’t receive OTP? {' '}
                                    <span className='resend_otp_text !text-[#0057E3]'>Resend</span>
								</button>
							) : (
